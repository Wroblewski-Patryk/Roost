...
export function CompanyObjectsWorkbench({ type, departmentKey }: { type: CompanyObjectType; departmentKey: CoreAreaKey }) {
  ...
  const packet = useOwnerPacket<CompanyObject[]>(`/v1/company-objects/${type}?departmentKey=${departmentKey}&includeCompanyWide=true&refresh=${refreshKey}`, true, t); const departmentPacket = useOwnerPacket<{ departments: Department[] }>(`/v1/departments?refresh=${refreshKey}`, true, t); const rows = packet.data || []; const labels = useTranslatedTableLabels();
  ...
  return <><CcPageHeader actions={<><CcButton href="/areas?area=00-ogolny&view=company-graph" iconLeft="ph-graph" size="sm" variant="outline">Company Graph</CcButton><CcButton iconLeft="ph-plus" onClick={() => setEditing(null)} size="sm" variant="primary">{polish ? "Utwórz" : "Create"}</CcButton></>} description={canonical ? (polish ? "Kanoniczny, ogólnofirmowy katalog. Filtry działowe pokazują perspektywy bez kopiowania danych." : "Canonical company-wide catalog. Department filters create perspectives without copying data.") : (polish ? "Kontekstowy widok wspólnego katalogu firmy, filtrowany relacjami organizacyjnymi." : "Contextual view of the shared company catalog, filtered by organizational relationships.")} eyebrow={departmentLabel(departmentKey, t)} title={titleFor(type, polish)} />
    ...
    {editing !== undefined ? <ObjectEditor departmentKey={departmentKey} departments={departmentPacket.data?.departments || []} onClose={() => setEditing(undefined)} onSaved={refresh} record={editing} type={type} /> : null}
