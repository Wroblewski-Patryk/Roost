...
export function AgentExecutionsRoute() {
  ...
  const [readyTaskId, setReadyTaskId] = useState<string | null>(null);
  const executionLabel = (execution: Execution) => execution.contextInvalidatedAt ? t(execution.status === "cancelled" ? "ready.contextAttemptClosed" : execution.contextStoppedAt ? "ready.contextStoppedStatus" : "ready.contextStoppingStatus") : humanizeBusinessValue(execution.status, undefined, locale);
  ...
  const columns = useMemo<Array<CcTableColumn<Execution>>>(() => [
    ...
    { key: "status", header: "Status", filterable: true, filterValue: (row) => row.status, cell: (row) => <span className={`badge ${tone(row.status)}`}>{executionLabel(row)}</span> },
  ...
  return <>
    ...
    {selected ? <div className="fixed inset-0 z-40 flex justify-end bg-neutral/55" role="dialog" aria-modal="true" aria-label="Codex execution details"><aside className="h-full w-full max-w-2xl overflow-y-auto bg-base-100 p-5 shadow-2xl">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2"><span className={`badge ${tone(selected.status)}`}>{executionLabel(selected)}</span>{activeStatuses.has(selected.status) && (!selected.contextInvalidatedAt || selected.contextStoppedAt) ? <CcButton loading={busy} onClick={() => void action("cancel")} className="min-h-11" size="sm" variant="outline">{selected.contextStoppedAt ? t("ready.closeStoppedAttempt") : polish ? "Anuluj" : "Cancel"}</CcButton> : null}{["failed", "cancelled"].includes(selected.status) ? selected.contextInvalidatedAt || selected.errorState?.retryable === false ? <CcButton onClick={() => setReadyTaskId(selected.task.id)} className="min-h-11" size="sm" variant="primary">{t("ready.reviewNewContext")}</CcButton> : <CcButton loading={busy} onClick={() => void action("retry")} size="xs" variant="primary">{polish ? "Ponów" : "Retry"}</CcButton> : null}</div>
      {selected.contextInvalidatedAt ? <section className="mt-4 grid gap-3"><CcNotice tone="warning" title={t(selected.contextStoppedAt ? "ready.contextStopped" : "ready.contextStopRequested")} detail={t(selected.contextStoppedAt ? "ready.contextStoppedHint" : "ready.contextStopRequestedHint")} /><ChangedContextSources sources={selected.contextInvalidation?.changedSources} /></section> : null}
    ...
    {readyTaskId ? <TaskReadinessModal taskId={readyTaskId} onClose={() => setReadyTaskId(null)} onSaved={() => setExecutionRefresh(value => value + 1)} /> : null}
  </>;
}
