          ...
          <aside className="roost-decision-inspector" aria-label={t("general.decisionContext")}>
            ...
            <section>
              ...
              <div className="roost-inspector-actions-list">
                {nextActions.slice(0, 3).map((action) => action.target ? <a href={action.target} key={action.key}><span><strong>{action.key==="review_decision_impact"?(locale==="pl"?"Sprawdź wpływ decyzji":"Review decision impact"):action.key==="resolve_material_questions"?(locale==="pl"?"Rozstrzygnij materialne pytania":"Resolve material questions"):action.label}</strong><small>{action.priority || t("state.normal")}</small></span><i className="ph-bold ph-arrow-up-right" aria-hidden="true"></i></a> : <span key={action.key}><strong>{action.key==="review_decision_impact"?(locale==="pl"?"Sprawdź wpływ decyzji":"Review decision impact"):action.key==="resolve_material_questions"?(locale==="pl"?"Rozstrzygnij materialne pytania":"Resolve material questions"):action.label}</strong><small>{action.priority || t("state.normal")}</small></span>)}
