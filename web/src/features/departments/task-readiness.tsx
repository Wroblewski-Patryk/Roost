  ...
  return <CcRecordEditorModal titleId="task-readiness-title" eyebrow={tr("title")} title={e?.task.title || tr("title")} description={tr("description")} closeLabel={tr("close")} onClose={close} onSubmit={submit} maxWidthClassName="max-w-5xl" actions={<>
    ...
    {packet && e && draft ? <>
      <section aria-label={tr("title")} className="grid gap-4 border-b border-base-300 pb-4">
        ...
        {packet.changedSources?.length ? <details className="text-sm" open><summary className="cursor-pointer font-bold">{tr("changedSources")} ({packet.changedSources.length})</summary><ul className="mt-2 grid gap-2">{packet.changedSources.map(source => <li className="min-w-0 break-words" key={`${source.table}:${source.id}`}><span className="font-medium">{source.label === source.table ? tr("changedSource") : source.label}</span><span className="text-company-muted"> · {tr(`change.${["insert", "update", "delete"].includes(source.operation) ? source.operation : "update"}`)}</span><div className="text-xs text-company-muted"><span className="block break-all">{source.id}</span><time dateTime={source.changedAt}>{new Date(source.changedAt).toLocaleString(locale)}</time></div></li>)}</ul></details> : null}
