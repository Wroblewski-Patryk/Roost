...
const optionKey=(o:any)=>`${o.operation}:${o.credentialId}${o.decision?":"+JSON.stringify(o.decision):""}${o.clarification?":"+JSON.stringify(o.clarification):""}${o.handoff?":"+JSON.stringify(o.handoff):""}`;
...
export function TaskCapabilityModal({ taskId, onClose }: { taskId: string; onClose: () => void }) {
  ...
  const mandateWindow = !option?.mandateExpiresAt || Date.parse(until) <= Date.parse(option.mandateExpiresAt);
  const currentInput = revoking ? { reason } : option && Number.isFinite(Date.parse(from)) && Number.isFinite(Date.parse(until)) ? { expectedVersion: data.expectedVersion, credentialId: option.credentialId, operation: option.operation, ...(option.clarification?{clarification:option.clarification,clarificationContextVersion:option.contextVersion}:{}), ...(option.handoff?{handoff:option.handoff}:{}), ...(option.decision?{decision:option.decision}:{}), validFrom: new Date(from).toISOString(), validUntil: new Date(until).toISOString(), reason } : null;
  ...
  async function submit(event: FormEvent) {
    event.preventDefault(); if (busy || !data || !currentInput || !revoking && (!option || (!validWindow || !mandateWindow) && !isRetry)) return;
  ...
  if(showRisk)return <TaskRiskModal taskId={taskId} onClose={()=>{setShowRisk(false);void load();}}/>;
  ...
  return <CcRecordEditorModal titleId="grant-title" title={c.title} eyebrow={data?.task.title} description={c.boundary} closeLabel={c.close} onClose={close} onSubmit={submit} maxWidthClassName="max-w-4xl" actions={<><CcButton variant="ghost" disabled={busy} onClick={close}>{c.close}</CcButton>{revoking ? <CcButton variant="warning" type="submit" disabled={busy || reason.trim().length < 3}>{c.confirm}</CcButton> : data?.options.length ? <CcButton variant="primary" type="submit" disabled={busy || !option || (!validWindow || !mandateWindow) && !isRetry || reason.trim().length < 3}>{c.issue}</CcButton> : null}</>}>
    <div className="grid min-w-0 gap-5 [overflow-wrap:anywhere]">
      ...
      <CcButton disabled={busy||dirty} onClick={()=>setShowRisk(true)} variant="outline">{locale==="pl"?"Ocena ryzyka":"Risk assessment"}</CcButton>
