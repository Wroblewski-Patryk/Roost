...
type GraphPacket = { schemaVersion: string; generatedAt: string; rootNodeId: string; nodes: GraphNode[]; edges: GraphEdge[]; organizationalMemberships: Array<{ entityType: string; entityId: string; departmentKey: string; role: string }> };
...
export function CompanyGraphRoute() {
  ...
  }, [activeFocus, allEdges, allRecords, enabledTypes, query, recordById]);
  ...
  const nodes = useMemo<Array<Node>>(() => {
    ...
    return visibleRecords.map((record) => ({ id: record.id, position: positions.get(record.id) ?? { x: 0, y: 0 }, data: { label: <div className="company-graph-node-copy"><small>{humanizeBusinessValue(record.recordType || record.entityType, undefined, locale)}</small><strong>{record.label}</strong><span>{record.entityType === "workspace" ? `${allRecords.length - 1} mapped records` : humanizeBusinessValue(record.state || "unknown", undefined, locale)}</span></div> }, className: `company-graph-node${selectedId === record.id ? " is-selected" : ""}${["workspace", "department"].includes(record.entityType) ? " is-anchor" : ""}`, style: { borderColor: typeColors[record.entityType] || "#64748b", borderLeftWidth: 5, width: record.entityType === "workspace" ? 250 : 220 }, zIndex: selectedId === record.id ? 2 : record.id === activeFocus?.id ? 1 : 0 }));
  ...
  const edges = useMemo<Array<Edge>>(() => {
    ...
    const stroke = edge.status === "blocked" ? "#dc626f" : connectedToSelection ? "#a7b7ff" : edge.source === "explicit" ? "#8ea5ff" : "#667694";
    const showLabel = connectedToSelection || visibleGraphEdges.length <= 16;
    return { id: edge.id, source: edge.from.entityId, target: edge.to.entityId, type: "straight", label: showLabel ? humanizeBusinessValue(edge.type) : undefined, animated: edge.status === "blocked", markerEnd: { type: MarkerType.ArrowClosed, color: stroke }, labelBgBorderRadius: 5, labelBgPadding: [5, 3], labelBgStyle: { fill: "#111827", fillOpacity: 0.94 }, labelStyle: { fill: "#d3daea", fontSize: 10, fontWeight: 700 }, style: { stroke, strokeDasharray: edge.source === "explicit" ? undefined : "5 4", strokeOpacity: connectedToSelection || !selectedId ? 0.9 : 0.2, strokeWidth: connectedToSelection ? 2.8 : edge.source === "explicit" ? 2 : 1.5 }, zIndex: connectedToSelection ? 1 : 0 };
    });
  }, [allEdges, selectedId, visibleIds]);
  ...
  }, [activeFocus?.id, flow, visibleRecords]);
  ...
  return <><CcPageHeader actions={<CcButton href="/areas?area=00-ogolny&view=overview" iconLeft="ph-gauge" size="sm" variant="outline">Company dashboard</CcButton>} description="Interactive, whole-workspace state graph. Filter layers, inspect typed relationships and open any canonical object without switching between mini-applications." eyebrow="00 General · Company intelligence" title="Company Graph" />
    <section className="roost-work-panel rounded-company overflow-hidden">
      <header className="company-graph-toolbar"><label className="company-graph-search"><i className="ph-bold ph-magnifying-glass" aria-hidden="true"></i><input aria-label="Search Company Graph" onChange={(event) => setQuery(event.target.value)} placeholder="Search objects and relationships…" type="search" value={query} /></label><div className="company-graph-health"><span className={`badge badge-outline company-graph-health--${health.data?.status || "unknown"}`}>{health.data ? `${health.data.score}% ${humanizeBusinessValue(health.data.status)}` : "Health…"}</span><span>{visibleRecords.length} nodes · {edges.length} relations{visibility.total > visibleRecords.length ? ` · ${visibility.total - visibleRecords.length} more` : ""}</span></div></header>
      ...
      {activeFocus ? <div className="company-graph-focusbar"><button disabled={activeFocus.id === graph.data?.rootNodeId} onClick={() => graph.data?.rootNodeId && focusNode(graph.data.rootNodeId)} type="button"><i className="ph-bold ph-buildings" aria-hidden="true"></i> Company map</button><i className="ph-bold ph-caret-right" aria-hidden="true"></i><strong>{activeFocus.label}</strong><span>Double-click a node to explore its direct relationships.</span></div> : null}
