...
const reportPath = path.join(root, "docs/status/architecture-node-links-report.json");
const registryFiles = [
...
function collectKnownNodeSlugs() {
  const set = new Set();
...
function main() {
  if (!fs.existsSync(generatedDir)) {
    const report = {
      ...
      status: "failed",
      reason: `Missing generated node directory: ${generatedDir}`,
      totalFiles: 0,
      brokenLinks: []
    };
  ...
  const knownNodeSlugs = collectKnownNodeSlugs();
  ...
  for (const fileName of files) {
    ...
    for (const match of matches) {
      ...
      const targetSlug = slug(rawTarget);
      if (!knownNodeSlugs.has(targetSlug)) {
        brokenLinks.push({
  ...
  if (brokenLinks.length) {
    console.error("Architecture node link gate failed:");
    for (const entry of brokenLinks.slice(0, 50)) {
  ...
  console.log(`Architecture node link gate passed: ${files.length} files, 0 broken links.`);
}
