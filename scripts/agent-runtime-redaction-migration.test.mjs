...
test("runtime redaction migration preserves historical content and authentication data", () => {
  const database = `companycore_test_redaction_${randomUUID().replaceAll("-", "")}`;
  ...
  try {
    ...
    if (!/^companycore_test_redaction_[a-f0-9]{32}$/.test(database)) throw new Error("Unsafe fixture database name");
