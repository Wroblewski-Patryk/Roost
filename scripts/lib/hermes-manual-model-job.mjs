// Separate manual-only Job protocol/receipt registry; never qualifies managed Roost.
...
export const windowsJobVersion = "hermes-manual-model-job-v1";
const source = fileURLToPath(new URL("../hermes-manual-model-job.cs", import.meta.url));
...
const gatedEventSchema = z.discriminatedUnion("type", eventSchema.options.map(schema => schema.extend({
  version: z.literal("hermes-manual-model-job-v2"), ...(schema.shape.type.value === "assigned" ? { challenge: uuid } : {}),
...
export async function buildWindowsJobLauncher(directory, { testFaults = false } = {}) {
  ...
  const executable = path.join(directory, testFaults ? "hermes-model-job-test.exe" : "hermes-model-job.exe");
...
export async function temporaryWindowsJobLauncher(run) {
  ...
  try { parent = await realpath(os.tmpdir()); directory = await mkdtemp(path.join(parent, "hermes-model-owned-job-")); }
  catch { throw fail(); }
  try { return await run(await buildWindowsJobLauncher(directory)); }
  finally {
    if (path.dirname(directory) !== parent || !path.basename(directory).startsWith("hermes-model-owned-job-")) throw fail();
...
export async function startWindowsJob(artifact, options) {
  ...
  const version = confirmResume === undefined ? windowsJobVersion : "hermes-manual-model-job-v2";
  ...
  if (Buffer.byteLength(encoded) > 262144 || !uuid.safeParse(attempt).success || !Number.isInteger(durationMs) || durationMs < 1 || durationMs > 14400000) throw fail();
  ...
  const completion = new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", chunk => {
      if (problem && (!assigned || version !== "hermes-manual-model-job-v2")) return;
      try {
    ...
    child.on("close", code => {
      ...
      const result = Object.freeze({ ...receipt, launcherSha256: artifact.sha256, sourceSha256: artifact.sourceSha256,
        executableDigest });
      if (assigned && (receipt.resumed || version === "hermes-manual-model-job-v2") && !build.testFaults) receipts.set(result, { at: performance.now() });
      if (problem) { const error = fail(); if (!receipt.resumed && version === "hermes-manual-model-job-v2") error.details = { ownedTreeReceipt: result }; reject(error); return; }
      resolve(result);
    });
