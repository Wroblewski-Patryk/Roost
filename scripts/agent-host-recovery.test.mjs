...
async function harness(t, stopStage) {
  ...
  async function run(stop) {
    const fake = `process.stdin.resume(); process.stdin.on('end',()=>{console.log(JSON.stringify({type:'turn.started'}));console.log(JSON.stringify({type:'item.completed',item:{type:'command_execution',command:'synthetic command',status:'completed',exit_code:0}}));console.log(JSON.stringify({type:'item.completed',item:{type:'agent_message',text:'Recovered fixture complete'}}));console.log(JSON.stringify({type:'turn.completed',usage:{input_tokens:10,output_tokens:5}}));});`;
    const script = `import cp from 'node:child_process'; import {syncBuiltinESMExports} from 'node:module'; const original=cp.spawn; cp.spawn=(command,args,options)=>{if(command!=='recovery-test-codex')return original(command,args,options); const child=original(process.execPath,['-e',${JSON.stringify(fake)}],options); process.stdout.write('FAKE_PID:'+child.pid+'\\n');return child;};syncBuiltinESMExports();await import('./scripts/fixtures/register-host-fixture-launch.mjs');const {runHost}=await import('./scripts/roost-codex-agent-host.mjs');const {acquireWriterLock}=await import('./scripts/lib/agent-host-writer-lock.mjs');await runHost({providerAdmissionForTest:()=>null,readTaskCommit:async()=>"a".repeat(40),readTaskBranch:async()=>${JSON.stringify(f.packet.contract.singleTask.branch)},createOutputBudget: (await import('./scripts/lib/agent-host-output-budget.mjs')).createObservedOutputBudget, acquireLock:(options)=>acquireWriterLock(${JSON.stringify(directory)},options),onCheckpoint:async(stage)=>{if(stage===${JSON.stringify(stop || "never")})process.exit(73);}});`;
...
for (const reason of ["lease_expired", "packet_changed", "context_changed", "repository_mismatch", "sandbox_invalid", "writer_locked"]) test(`restart stops on ${reason}`, { skip: process.platform !== "win32", timeout: 15000 }, async (t) => {
  ...
  if (reason === "repository_mismatch") { h.config.repositories.demoapp.originUrl = "https://github.com/example/other.git"; await writeFile(h.configPath, JSON.stringify(h.config)); }
