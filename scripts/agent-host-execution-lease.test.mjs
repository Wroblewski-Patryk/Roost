...
test(sandboxBlocked ? "the real host rejects unrestricted sandbox before claiming work" : "the real host stops before spawn and never completes or reclaims after initial rejection", { skip: process.platform !== "win32", timeout: 15_000 }, async () => {
  ...
  const server = createServer((req, res) => {
    ...
    const data = req.url.startsWith("/v1/agent-runtime/recovery?") ? { executions: [], executionEnabled: true } : req.url.endsWith("/register") ? compatibleHostFixture()
      : req.url.endsWith("/claim") ? { id: "execution", taskId: "task", applicationId: "app", leaseToken: "test-only", task: { title: "test" }, application: { id: "app", name: "DemoApp", slug: "demoapp", repositories: [{ url: "https://github.com/example-org/DemoApp.git", isPrimary: true }] } }
      : {};
  ...
  try {
    ...
    const startHost = `import { runHost } from './scripts/roost-codex-agent-host.mjs'; import { acquireWriterLock } from './scripts/lib/agent-host-writer-lock.mjs'; await runHost({providerAdmissionForTest:()=>null, acquireLock: () => acquireWriterLock(${JSON.stringify(directory)}) });`;
