// Manual model operation only: separate protocol and four-hour ceiling.
// Managed Roost native Job source/policy remains unchanged.
using System;
using System.IO;
using System.Text;
...
internal static class HermesManualModelJob
{
    static string Version = "hermes-manual-model-job-v1";
    ...
    static int Main()
    {
        ...
        try {
            ...
            Need(Version == "hermes-manual-model-job-v1" || Version == "hermes-manual-model-job-v2");
            ...
            if (fixedEffect) { keys.Add("fixedEffect"); Need(Version == "hermes-manual-model-job-v2" && request["fixedEffect"] is bool && (bool)request["fixedEffect"]); }
            ...
            duration = Number(request, "durationMs", 1, 14400000); stopMs = Number(request, "stopMs", 100, 3000);
...
bool member; Need(IsProcessInJob(process, job, out member) && member && Active() == 1); assigned = true;
            ...
            if (Version == "hermes-manual-model-job-v2") assignment.Add("challenge", challenge);
            Need(Emit(assignment));
            if (Version == "hermes-manual-model-job-v2") {
