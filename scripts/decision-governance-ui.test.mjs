...
try{
 for(const locale of ['pl','en'])for(const width of [390,768,1440])for(const mode of ['create','accept','stale','readonly','defer','reopen','interview-reopen']){
  ...
  const root=()=>({expectedVersion:hash,canWrite:mode!=='readonly',catalog:[{type:'task',id:task,title:'Parser fixture task'}],resourceCatalog:[{id:task,name:'Fixture capacity'}],configurationCatalog:[],revisions:[{id,title:body.title,state}],deferrals:mode.includes('reopen')?[{id:task,targetId:id,targetType:'interview',reason:'budget',explanation:'Wait for confirmed capacity',condition:{type:'owner_signal'},...(reopened?{eventId:id,reopenedAt:'2026-09-08T11:00:00Z'}:{})}]:[],truncated:false});
