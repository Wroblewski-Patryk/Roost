...
try {
  for (const locale of ["pl", "en"]) for (const width of [390, 834, 1440]) for (const state of ["direct_codex", "hermes_codex", "unknown", "empty", "error"]) {
    ...
    const report = contract.projectProvider({ kind: state, blockers: state === "hermes_codex" ? ["hermes_disabled", "hermes_executable_missing"] : [] });
    ...
    if (["hermes_codex", "direct_codex"].includes(state)) await page.screenshot({ path: path.join(output, `${locale}-${state}-${width}.png`), fullPage: true });
