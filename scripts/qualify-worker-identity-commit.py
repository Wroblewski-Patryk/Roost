...
parser.add_argument('--suite', choices=['lifecycle','issuer','channel','ticket','revocation','attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority'], default='lifecycle')
...
def run(command, source=None):
    ...
    if result.returncode:
        if args.suite in ('channel','ticket','revocation','attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority') and any(str(v).startswith('companycore_test_identity_') for v in command):
...
try:
    ...
    env['PROBE_FAULTS'] = '1' if args.scenario=='full' or args.suite in ('issuer','channel','ticket','revocation','attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority') else '0'
    env['PROBE_REGISTRATION'] = '1' if args.registration_probe or args.registration_matrix or args.suite in ('attested-reader','dispatch','completion','channel-revoke','proof-authority') else '0'
    ...
    suite = {'issuer':'bootstrap-issuer-native','channel':'bootstrap-channel-native','lifecycle':'worker-identity-lifecycle-native','ticket':'bootstrap-ticket-native','revocation':'bootstrap-ticket-revocation-native','attestation':'decision-attestation-native','attested-reader':'bootstrap-attested-reader-native','dispatch':'bootstrap-durable-dispatch-native','completion':'bootstrap-canonical-completion-native','channel-revoke':'bootstrap-channel-revocation-native','proof-authority':'bootstrap-proof-authority-native'}[args.suite]
    native_code = preamble+("require('tsx/cjs');require('./src/tests/"+suite+".test.ts');" if args.suite in ('attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority') else "require('./dist/tests/"+suite+".test.js');")
    def native_run():
        ...
        try: child.wait(timeout=900 if args.suite in ('channel','ticket','revocation','attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority') else 240)
    ...
    if result.returncode and args.suite in ('channel','ticket','revocation','attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority'):
        ...
        while native_runs<(8 if args.suite in ('attestation','proof-authority') else 3) and input().strip()=='retry':
            native_runs += 1
            ...
            if args.suite in ('completion','proof-authority'):
                ...
                assert len(candidate)==(86 if args.suite=='proof-authority' else 85) and candidate[:-1]==original_chain[:-1]
    ...
    if args.scenario=='full' or args.suite in ('issuer','channel','ticket','revocation','attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority'):
        expected = 23 if args.suite=='proof-authority' else 16 if args.suite=='channel-revoke' else 20 if args.suite=='completion' else 24 if args.suite=='dispatch' else 18 if args.suite=='attested-reader' else 24 if args.suite=='attestation' else 13 if args.suite=='revocation' else 17 if args.suite=='ticket' else 19 if args.suite=='channel' else 17 if args.suite=='issuer' else 16
        assert re.search(r'^# tests '+str(expected)+'$',result.stdout,re.M),'Full suite count differs'
        time.sleep(.1)
        expected_cuts = cuts_before_last_run+(3 if args.suite=='attestation' else 2 if args.suite in ('dispatch','completion','channel-revoke','proof-authority') else 1) if args.suite in ('revocation','attestation','attested-reader','dispatch','completion','channel-revoke','proof-authority') else native_runs if args.suite=='channel' else 1
    ...
    if args.suite in ('attested-reader','dispatch','completion','channel-revoke','proof-authority'): assert final_chain==chain,'Applied and final source chains must match'
    if args.suite=='dispatch': assert len(chain)==84,'Dispatch qualification requires the complete 84-migration chain'
    if args.suite in ('completion','channel-revoke'): assert len(chain)==85,'Completion qualification requires the complete 85-migration chain'
    if args.suite=='proof-authority': assert len(chain)==86,'Proof qualification requires the complete 86-migration chain'
