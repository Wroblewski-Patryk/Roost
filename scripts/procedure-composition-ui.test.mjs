...
try {
 for(const locale of ["pl","en"])for(const width of [390,768,1440])for(const mode of ["selection","conflict","exception","readonly","publish","published-readonly"]){
  ...
  if(published)await dialog.getByText(locale==="pl"?"Wrażliwe etykiety katalogu zostały ukryte. Treści kontraktów nadal wymagają pełnej walidacji.":"Sensitive catalog labels have been hidden. Contract content still requires full validation.",{exact:true}).waitFor();
