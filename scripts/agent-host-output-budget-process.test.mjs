...
const scenarios = ["hostMaintenance", "unsupported", "missing", "zero", "negative", "tooLarge", "beforeSpawn", "afterSpawn", "invalidUsage", "valid", "reportUnavailable", "recoverClaimed", "recoverPrepared"];
for (const scenario of scenarios) test(`output budget process containment: ${scenario}`, { skip: process.platform !== "win32", timeout: 20000 }, async () => {
  ...
  async function run(stopStage) {
    ...
    const script = `import cp from 'node:child_process';import{syncBuiltinESMExports}from'node:module';const original=cp.spawn;cp.spawn=(command,args,options)=>{if(command==='output-budget-fixture'){const c=original(process.execPath,['-e',${JSON.stringify(fake)}],options);console.log('WORKER:'+c.pid);c.stderr.on('data',d=>process.stdout.write(d));return c;}if(command==='taskkill.exe')console.log('TREE_STOP');return original(command,args,options);};syncBuiltinESMExports();await import('./scripts/fixtures/register-host-fixture-launch.mjs');const{runHost}=await import('./scripts/roost-codex-agent-host.mjs');const{acquireWriterLock}=await import('./scripts/lib/agent-host-writer-lock.mjs');const{createObservedOutputBudget}=await import('./scripts/lib/agent-host-output-budget.mjs');const{default:lifecycle}=await import('./scripts/lib/agent-host-lifecycle.cjs');await runHost({providerAdmissionForTest:()=>null,readTaskCommit:async()=>"a".repeat(40),readTaskBranch:async()=>${JSON.stringify(f.packet.contract.singleTask.branch)},${factory}acquireLock:options=>acquireWriterLock(${JSON.stringify(directory)},options),onCheckpoint:stage=>{if(stage==="prepared" && ${JSON.stringify(scenario)}==="hostMaintenance")throw lifecycle.lifecycleError({checkedAt:new Date().toISOString(),engine:"available",workloads:"continuous",distribution:"running_healthy",proxy:"unavailable",symptom:"distribution_proxy_exit"});if(stage===${JSON.stringify(stopStage)})process.exit(73);}});`;
  ...
  try {
    ...
    if (scenario === "valid") {
      ...
      if (scenario === "hostMaintenance") { assert.equal(lock.checkpoint.stage, "prepared"); assert.equal(fail.input.details.decision.status, "required"); assert.equal(fail.input.details.health.engine, "available"); }
      const expected = scenario === "hostMaintenance" ? "host_maintenance_required" : invalid ? "execution_packet_invalid" : ["afterSpawn", "beforeSpawn", "reportUnavailable"].includes(scenario) ? "agent_execution_output_budget_exceeded"
