...
async function importGoogleDriveFolders(input: {
  ...
  for (const file of files) {
    ...
    const contentChanged = !previous || snapshotMetadata?.snapshotSchemaVersion !== 3 || snapshotMetadata.partial === true || previous.scanStatus !== "completed" || previous.headRevisionId !== (file.headRevisionId ?? null)
