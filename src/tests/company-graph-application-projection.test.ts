...
test("projects the application slice under Innovation while reusing native company nodes", () => {
  const packet: ApplicationGraphPacket = {
    ...
    nodes: [
      ...
      { id: "application:app-1", entityId: "app-1", type: "application", label: "DemoApp", shortLabel: "DemoApp", category: "Application", status: "active", completeness: 50, isRequired: true, isBlocked: false, hasEvidence: true, tags: [], parentNodeId: "portfolio:workspace-1", childCount: 1, path: ["portfolio:workspace-1", "application:app-1"], details: {} },
  ...
  const result = projectApplicationPacketsIntoCompanyGraph({ workspaceId: "workspace-1", workspaceName: "Example Company", innovationDepartmentNodeId: "department:innovation", existingNodeIds: new Set(["app-1"]), packets: [packet] });
