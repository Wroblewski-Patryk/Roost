...
function project(body: any, all: boolean, state: RedactionState, allowLease: boolean) {
  ...
  function visit(value: any, depth: number): any {
    ...
    if (value.executionId && value.payload || value.checkpointVersion !== undefined && value.taskId || value.metadata?.executionId || /^(?:agent_execution|task_execution|task_review|task_capability|task_handoff)/.test(value.type ?? "")) return inspectRuntime(value, "read.shared_native", "diagnostic", { recordId: safeRuntimeId(value.id), executionId: safeRuntimeId(value.executionId ?? value.metadata?.executionId) }).value;
