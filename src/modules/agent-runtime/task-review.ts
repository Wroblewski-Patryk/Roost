...
const eligibleMember = (m: any) => m && ["owner", "admin", "member"].includes(m.role);
const decisionView = (d: any) => d ? { id: d.id, executionId: d.executionId, materialVersion: d.materialVersion, decision: d.decision, evidence: d.evidence, verifierId: d.verifierId, verifierLabel: object(d.snapshot).labels?.verifier, actorUserId: d.actorUserId, createdAt: d.createdAt, action: d.action ? actionView(d.action) : null } : null;
const actionView = (a: any) => ({ id: a.id, action: a.action, actorUserId: a.actorUserId, managerId: a.managerId, managerLabel: object(a.snapshot).managerLabel, childTaskId: a.childTaskId, correction: a.correction, createdAt: a.createdAt });

async function state(db: Db, workspaceId: string, taskId: string, actorUserId?: string) {
  ...
  const member = actorUserId ? await db.workspaceMembership.findFirst({ where: { workspaceId, userId: actorUserId } }) : null;
  const roleMatches = (role: any) => eligibleMember(member) && role?.type === "human" && role.principal?.kind === "user" && role.principal.id === actorUserId;
  ...
  return { task, execution, result, contract, authorities, labels, decision, materialVersion, expectedVersion, current, roleIssues,
...
export async function taskReviewView(db: Db, workspaceId: string, taskId: string, actorUserId?: string, cursor?: string) {
  const s = await state(db, workspaceId, taskId, actorUserId);
...
export async function recordTaskReview(db: Db, workspaceId: string, taskId: string, actorUserId: string, body: unknown) {
  const input = reviewDecisionSchema.parse(body), s = await state(db, workspaceId, taskId, actorUserId);
  if ("error" in s) return s;
  const requestHash = reviewDigest({ input, actorUserId, taskId });
  ...
  const member = await db.workspaceMembership.findFirst({ where: { workspaceId, userId: actorUserId } });
  if (eligibleMember(member) && !prior && !s.current) return { error: "task_review_stale" };
  if (!eligibleMember(member) || s.authorities.verifier?.principal?.id !== actorUserId || s.roleIssues.length) return { error: "task_review_forbidden" };
  ...
  const saved = await db.taskReviewDecision.create({ data: { workspaceId, taskId, executionId, requestId, requestHash, materialVersion, actorUserId,
  ...
  await db.event.create({ data: { workspaceId, taskId, type: "task_review_decided", source: "roost", resourceType: "task_review", resourceId: saved.id, payload: { reviewId: saved.id, executionId, materialVersion, decision, actorUserId } } });
  return { decision: decisionView(saved), replayed: false };
}

export async function actOnTaskReview(db: Db, workspaceId: string, taskId: string, actorUserId: string, body: unknown) {
  const input = reviewActionSchema.parse(body), s = await state(db, workspaceId, taskId, actorUserId);
  if ("error" in s) return s;
  const requestHash = reviewDigest({ input, actorUserId, taskId });
  ...
  if (!eligibleMember(member) || manager?.principal?.id !== actorUserId || s.roleIssues.length) return { error: "task_review_forbidden" };
  ...
  const action = await db.taskReviewAction.create({ data: { workspaceId, taskId, reviewId: input.reviewId, requestId: input.requestId, requestHash, actorUserId, managerId: manager!.id, action: input.action, childTaskId, dependencyId, correction,
  ...
  await db.event.create({ data: { workspaceId, taskId, type: "task_review_manager_action", source: "roost", resourceType: "task_review_action", resourceId: action.id, payload: { actionId: action.id, reviewId: input.reviewId, action: input.action, actorUserId, childTaskId, dependencyId } } });
