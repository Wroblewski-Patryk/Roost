...
export function ownerTicketHandler(action: keyof Service, service?: Service) {
  return async (req: Request, res: Response) => {
    ...
    const owner = req.auth?.authType === "user" && req.auth.workspaceRole === "owner" && !!req.auth.userId;
    const worker = ["consume", "status"].includes(action) && req.auth?.authType === "api_key" && !req.auth.agentId && !req.auth.userId && !!req.auth.workerTicketIdentity;
    if (!owner && !worker) return sendApiError(res, 403, "owner_ticket_forbidden");
