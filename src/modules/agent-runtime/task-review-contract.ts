...
const precondition = { requestId: z.string().uuid(), expectedVersion: z.string().regex(/^[a-f0-9]{64}$/), executionId: z.string().uuid(), materialVersion: z.string().regex(/^[a-f0-9]{64}$/) };
...
export const reviewActionSchema = z.discriminatedUnion("action", [
  z.object({ requestId: precondition.requestId, expectedVersion: precondition.expectedVersion, reviewId: z.string().uuid(), action: z.literal("return_to_executor"), scope: lines }).strict(),
  z.object({ requestId: precondition.requestId, expectedVersion: precondition.expectedVersion, reviewId: z.string().uuid(), action: z.literal("create_specialist_task"), scope: lines, specialist: z.object({ id: z.string().uuid(), revision: z.string().min(1).max(100) }).strict() }).strict()
]);
