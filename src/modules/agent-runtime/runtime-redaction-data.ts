...
const fields: Record<string, string[]> = {
  ...
  TaskAdmissionScope:["input"], TaskAdmissionEvidence:["detail"],
...
export function installRuntimeRedaction(client: PrismaClient) {
  client.$use(async (params, next) => {
    if (!redactionState.getStore()?.flushing && /^(?:create|createMany|update|updateMany|upsert)$/.test(params.action)) {
      ...
      const nativeEvent = params.model === "Event" && !params.action.startsWith("create") && await client.event.count({ where: { ...params.args?.where, OR: ["agent_execution", "task_execution", "task_review", "task_capability", "task_handoff", "task_clarification", "task_interview", "decision_governance"].map(prefix => ({ type: { startsWith: prefix } })) } }) > 0;
      for (const data of [params.args?.data, params.args?.create, params.args?.update].flat().filter(Boolean)) {
        ...
        for (const relation of ["admissionTargets","admissionReleases","admissionScopes","admissionEvidence",...(params.model === "TaskAdmissionScope" ? ["evidence"] : []),"riskScopes","riskAssessments","nativeSuspensions", "suspensionJournal", ...(params.model === "NativeCapabilitySuspension" ? ["journal"] : []), ...(params.model === "NativeSuspensionJournal" ? ["references","evidence"] : []), "agentExecutions", "agentExecutionEvents", "evidenceRecords", "agentLogs", "capabilityGrants", "reviewDecisions", ...(params.model === "AgentHost" ? ["executions"] : []), ...(params.model === "AgentExecution" ? ["events"] : [])]) {
        ...
        if (params.model === "Event" && (nativeEvent || /^(?:agent_execution|task_execution|task_review|task_capability|task_handoff|task_clarification|task_interview|decision_governance)/.test(data.type ?? ""))) names = ["payload"];
