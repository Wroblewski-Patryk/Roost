...
export function installRuntimeRedaction(client: PrismaClient) {
  client.$use(async (params, next) => {
    if (!redactionState.getStore()?.flushing && /^(?:create|createMany|update|updateMany|upsert)$/.test(params.action)) {
      ...
      const nativeEvent = params.model === "Event" && !params.action.startsWith("create") && await client.event.count({ where: { ...params.args?.where, OR: ["agent_execution", "task_execution", "task_review", "task_capability", "task_handoff", "task_clarification"].map(prefix => ({ type: { startsWith: prefix } })) } }) > 0;
