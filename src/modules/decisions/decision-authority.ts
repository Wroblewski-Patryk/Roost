...
export async function decisionAuthority(db: Db, workspaceId: string, body: any, impact: any, loaded?: Awaited<ReturnType<typeof authorityState>>, operation?: string) {
  ...
  if (!body.authority || body.workerCredential || body.workerTransport || body.workerBootstrap) return { status: "owner_reserved", reason: body.workerBootstrap ? "worker_bootstrap_admission" : body.workerTransport ? "worker_transport_admission" : body.workerCredential ? "worker_credential_lifecycle" : "unclassified_owner", principal: { kind: "user", id: state.ownerUserId }, path: [], mandate: null };
