...
export interface BootstrapTx{
  ...
  transition(id:string,from:BootstrapAttempt["state"],to:BootstrapAttempt["state"]):Promise<boolean>;
}
...
function signed(kind:"ticket"|"decision"|"peer"|"completion",value:SignedTransport<unknown>,c:BootstrapContext){
...
export function createWorkerBootstrapService(store?:BootstrapStore,exchange?:BootstrapExchange,clock=()=>new Date()){
  async function authorize(tx:BootstrapTx,ticketId:string,phase:"initial"|"reserved"|"complete"){
    ...
    if(!c||!c.ownerActive||!c.hostActive||!c.installationActive||c.ownerId!==ticket.ownerId||!same(c.binding,i.binding)||!same(c.channel,i.channel)||!signed("ticket",row!.signed,c))deny("binding_changed");
  ...
  function peerMatches(value:SignedTransport<BootstrapPeer>,ticket:BootstrapTicket,c:BootstrapContext,attempt:BootstrapAttempt){
      ...
      try{
        ...
        const pending=exchange!.run({permit,deviceProof:input.deviceProof,beforeSend:async peer=>{
          ...
          try{return await store!.transaction(async tx=>{
            ...
            if(!live()||!await tx.transition(a!.id,"consumed","dispatched"))deny("replay");approvedPeer=structuredClone(peer);return true;
        ...
        const completed=await store!.transaction(async tx=>{
          ...
          if(!await tx.transition(a!.id,"dispatched","acknowledged"))deny("replay");
