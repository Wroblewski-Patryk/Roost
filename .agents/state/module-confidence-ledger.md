...
| --- | --- | --- | --- |
| Protected Product Map projection read after rejected old replay | verified locally, protected deployment pending | `.codex/tasks/2026-08-08-product-map-out-of-order-readback.md`; targeted Node test PASS (8/8); `npm run build:server` PASS. Out-of-order ingress remains rejected/audited while only a real same-snapshot/different-digest quarantine invalidates active LKG state; a different offering's SHA mismatch remains item-scoped. | Deploy the exact Roost SHA, publish a fresh Paperclip projection, and run the protected fail-closed readback. |
