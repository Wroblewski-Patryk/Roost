...
## Header
- ID: LUC-1197
- Title: Unclassified user workflow `USE /deals` proof-link closure
- Task Type: fix
...
- Iteration: 2026-07-15-LUC-1197
- Operation Mode: TESTER
- Mission ID: LUC-1197-USE-DEALS-PROOF-LINK
...
- Mission objective: close the routed Project Truth `missing_test_link` gap for
  `src/app.ts#/deals` by linking the existing `/v1/deals` API proof to the
...
- Explicit exclusions: no route logic change, no auth model change, no deploy,
  no production mutation, no new browser work, and no broader sales or finance
  redesign.
...
## Context
Project Truth currently routes `src/app.ts#/deals` (`USE /deals`) as the first
unclassified `missing_test_link` item. The behavior is already exercised in
`src/tests/api.test.ts` through direct `POST /v1/deals`, `GET /v1/deals/:id`,
`PATCH /v1/deals/:id`, `DELETE /v1/deals/:id`, and foreign-workspace isolation
assertions inside the protected API flow. The open gap is evidence-link drift
between the route mount and existing automated proof.

## Goal
Attach existing test evidence to `src/app.ts#/deals` in scanner metadata so the
generated app-completion and Project Truth outputs no longer classify it as
`missing_test_link`.
...
## Implementation Plan
1. Verify the dispatched route symbol and existing `/v1/deals` proof in code
   and generated status artifacts.
2. Link `src/app.ts#/deals` to the existing API proof through a minimal scanner
   override.
...
- `docs/architecture/scanner-overrides.json` contains a `verified` entry for
  `src/app.ts#/deals` with existing API proof evidence.
- Focused local proof confirms the current `/v1/deals` assertions still pass
  without runtime modification.
- Generated app-completion no longer reports
  `api_endpoint:use-deals:2ceaef3b27` as `missing_test_link`.
- Project Truth first gap moves away from `USE /deals`.
...
- Project Truth first gap: Unclassified user workflow `USE /deals`.
- Evidence status: `src/tests/api.test.ts` already proves create, read, update,
  archive, and workspace isolation behavior for `/v1/deals`.
- Closure risk: route-mount evidence drift, not missing runtime behavior.

### 2. Select Priority Mission Objective
- Selected mission: `USE /deals` missing-test-link closure for the exact
  `src/app.ts#/deals` mount.
- Deferred scope: no behavioral additions and no broader deals feature work.

### 3. Execute Implementation
- Scanner override links `src/app.ts#/deals` to existing proof in
  `src/tests/api.test.ts`, `src/modules/deals/deals.routes.ts`, and this task
  packet.
...
- Scope: completed the exact `missing_test_link` proof-link closure for
  `src/app.ts#/deals` only.
- Evidence:
  - `docs/architecture/scanner-overrides.json` marks `src/app.ts#/deals`
    `verified` through `src/tests/api.test.ts`,
    `src/modules/deals/deals.routes.ts`, and this task packet.
  - Focused protected API proof PASS after local build, migrations, and seed:
    ...
    `companycore-test-postgres-luc1197` on port `55434`; container removed
    during cleanup.
  - `npm run architecture:refresh` PASS.
  - External architecture-awareness refresh generated
    `2026-07-15T01:06:46.441Z` with `3042` entities / `7750` relations /
    `16523` files and materialized the exact proof relations.
  - Sequential app-completion refresh PASS with `46` items / `4` flows /
    `27` missing test links / `2` missing doc links / `0`
    implemented-needs-proof / `0` blocked; `USE /deals` no longer appears as
    `missing_test_link` and now resolves as the same-symbol docs-owned
    `missing_doc_link`.
  - Sequential Project Truth apply generated `2026-07-15T01:06:51.062Z` with
    ...
    `src/app.ts#/decisions` as `missing_test_link`.
...
- Source-control closure:
  - Local commit SHA: none in this heartbeat.
  - Exact no-commit blocker: the successful proof-link refresh produced the
    expected generated docs/status packet in a still-dirty worktree.
  - Source-control sidecar: [LUC-1200](/LUC/issues/LUC-1200) closed this
    coherent packet with a local commit.
- Residual risk: the route-level QA proof gap for `src/app.ts#/deals` is
  closed, but the same symbol is now docs-owned `missing_doc_link`; the next
  QA proof lane is unclassified `src/app.ts#/decisions`.
