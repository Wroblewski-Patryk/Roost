...
## Header
- ID: LUC-1312
- Title: Prove unclassified user workflow missing-doc-link for `USE /interactions`
- Task Type: research
- Current Stage: verification
- Status: DONE
- Owner: Product Docs
- Depends on: [LUC-1307](/LUC/issues/LUC-1307)
- Priority: P1
- Coverage Ledger Rows: Unclassified user workflow `src/app.ts#/interactions` `missing_doc_link`
- Module Confidence Rows: `Interactions documentation linkage`
- Requirement Rows: not applicable
- Quality Scenario Rows: not applicable
- Risk Rows: unclassified interactions route documentation-link drift
- Iteration: 2026-07-16-LUC-1312
- Operation Mode: BUILDER
- Mission ID: LUC-1312-UNCLASSIFIED-USE-INTERACTIONS-DOC-LINK
- Mission Status: DONE

## Process Self-Audit
- [x] All seven autonomous loop steps are represented.
- [x] Exactly one priority task is selected.
- [x] The task aligns with Project Truth, app-completion, and unclassified route source-of-truth files.
...
## Mission Block
- Mission objective: prove `src/app.ts#/interactions` with the smallest supported documentation-link evidence and link that proof into Project Truth.
- Release objective advanced: Roost Project Truth / app-completion confidence for the unclassified interactions route.
- Included slices: exact documentation relation, accepted interactions API contract readback, generated architecture/app-completion/Project Truth refresh, and state updates.
...
- Stop conditions: architecture refresh fails, generated readback keeps the exact symbol as `missing_doc_link`, or the accepted interactions API contract proves too weak to cover the mount without runtime changes.
- Handoff expectation: close [LUC-1312](/LUC/issues/LUC-1312) when `missing_doc_link` clears; route any next non-doc follow-up to the proper owner from Project Truth.

## Responsibility Lanes

| Lane | Owner | Source docs/state | Owned files/surfaces | Output | Validation/proof | Status |
| --- | --- | --- | --- | --- | --- | --- |
| Coordinator | Active chat | `AGENTS.md`, Paperclip wake payload | Task closure and source-of-truth updates | LUC-1312 packet and issue disposition | Final issue update | COMPLETE |
...
## Context
[LUC-1307](/LUC/issues/LUC-1307) already proved the routed `src/app.ts#/interactions` mount with the existing protected API suite and moved the same symbol from `missing_test_link` to `missing_doc_link`. `docs/API.md` already contains the accepted `/v1/interactions` and `/interactions` contract, but `docs/architecture/relations/documentation-links.csv` did not yet include the exact route-mount relation for `src/app.ts#/interactions`.

## Goal
Clear the exact `src/app.ts#/interactions` `missing_doc_link` row without changing runtime behavior.
...
## Implementation Plan
1. Confirm the generated first gap is the exact `src/app.ts#/interactions` `missing_doc_link` row.
2. Confirm the accepted interactions API contract already documents the protected `/v1/interactions` route family closely enough to cover the mount.
...
- Issues: Project Truth routes `src/app.ts#/interactions` as the current
  unclassified `missing_doc_link` gap.
- Gaps: the accepted Interactions API docs already describe the protected route
...
- Sources scanned: `src/app.ts`, `src/modules/interactions/interactions.routes.ts`,
...
- Logic: attach the exact protected route-mount entity to the existing
  accepted Interactions API contract.
...
- Failure trigger: if the row persists, inspect whether the documentation
  relation or current Interactions contract coverage is still too weak.

### 4. Execute Implementation
- Implementation notes: the documentation-links registry now links the exact
  `src/app.ts#/interactions` mount to the accepted `docs/API.md` Interactions
  contract that already covers `/v1/interactions`, `/interactions`,
  workspace scoping, client visibility enforcement, and archive-on-delete
  behavior.
...
- Result: PASS. Refreshed architecture-awareness materialized the exact
  ...
  Project Truth advances the first routed gap to `src/app.ts#/mcp`
...
- Simpler option considered: add more prose to `docs/API.md`; rejected because
  the accepted Interactions API contract already exists and the missing exact
...
- [x] The linked interactions API contract already documents the `/interactions` and `/v1/interactions` route family.
- [x] Architecture refresh applies the new documentation relation.
- [x] App-completion readback no longer reports `src/app.ts#/interactions` as `missing_doc_link`.
- [x] Project Truth first gap advances away from `src/app.ts#/interactions` and is explained with exact evidence.
...
- [x] Paperclip issue [LUC-1312](/LUC/issues/LUC-1312) receives a final disposition.

## Deliverable For This Stage
A verified local documentation-link closure packet for the exact
`src/app.ts#/interactions` route mount, plus refreshed generated truth and
...
- Manual checks: reviewed `docs/API.md`, `src/app.ts`, and the refreshed generated
  readbacks to confirm the exact mount now resolves to accepted Interactions contract evidence.
...
- Real API/service path used: yes; generated readback is anchored to the real `src/app.ts#/interactions` mount and accepted Interactions route contract.
...
- Regression check performed: yes; refreshed generated truth advanced the first routed gap away from `src/app.ts#/interactions`.

## Result Report
- Added a new `docs/architecture/relations/documentation-links.csv` row
  linking `src/app.ts#/interactions` to `docs/API.md`.
- Reused the existing Interactions API contract in `docs/API.md`; no contract
  text expansion was required because the accepted `/interactions` /
  `/v1/interactions` route family was already explicit.
- Architecture-awareness refresh PASS generated `2026-07-16T14:23:39.591Z`
  with `3079` entities / `8065` relations / `16523` files and materialized the
  exact `documents` relation.
- App-completion refresh PASS generated `2026-07-16T14:24:06.576Z` with
  `46` items / `4` flows / `20` missing test links / `1` missing doc link /
  `0` implemented-needs-proof / `0` blocked / `21` risk items, and no longer
  reports `api_endpoint:use-interactions:eb228af9f5` as `missing_doc_link`.
- Project Truth apply PASS generated `2026-07-16T14:24:07.447Z` with public
  probes `pass`, runtime findings `0`, incomplete event chains `0`,
  operational gate gaps `0`, and advances the first routed gap to
  `src/app.ts#/mcp` `missing_test_link` while leaving
...
- Files changed: `docs/architecture/relations/documentation-links.csv`,
  `.codex/tasks/luc-1312-prove-unclassified-user-workflow-missing-doc-link-for-use-interactions.md`,
  `.codex/tasks/luc-1312-completion-evidence.md`, generated `docs/graphs/*`,
  generated `docs/status/*`, and the canonical repo state summaries updated
  for this routed gap change.
- Residual risk: no remaining Documentation Steward action is needed for
  `src/app.ts#/interactions`; the next routed proof gap is QA-owned on
  `src/app.ts#/mcp`, and the only remaining docs-owned route gap is
