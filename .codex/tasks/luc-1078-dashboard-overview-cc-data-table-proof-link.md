...
## Header
- ID: LUC-1078
- Title: Prove Dashboard overview missing-test-link for cc-data-table-tsx
- Task Type: fix
- Current Stage: verification
- Status: DONE
- Owner: QA/Test
- Depends on: [LUC-998](/LUC/issues/LUC-998), [LUC-1076](/LUC/issues/LUC-1076)
- Priority: P1
...
- Mission ID: LUC-1078-dashboard-cc-data-table-proof-link
- Mission Status: DONE

## Goal
Close the generated Dashboard overview `missing_test_link` gap on
`web/src/components/cc-data-table.tsx` by linking the existing focused
dashboard/public-home browser proof to the shared managed-table component and
the dashboard-specific render helper it exercises, without introducing a
duplicate UI harness.
...
## Implementation Plan
1. Confirm the current Dashboard overview gap is limited to the shared
   `cc-data-table.tsx` component family.
2. Reuse the existing focused browser proof from `LUC-998` rather than adding
   a duplicate component-only harness.
3. Add explicit verification overrides for `cc-data-table.tsx`, `CcDataTable`,
   and the dashboard-rendered `filterBar` helper.
4. Rerun the smallest relevant refresh chain so app-completion and Project
   Truth stop routing `cc-data-table.tsx` as the first gap.
...
2. Added direct `tests` proof-link overrides from the existing `LUC-998`
   ...
   - `web/src/components/cc-data-table.tsx#CcDataTable`
   - `web/src/components/cc-data-table.tsx#filterBar`
3. Tightened the `CcDataTable` and `filterBar` scanner override descriptions so
   the app-completion heuristic reads the reused proof as explicit
   Playwright-style browser test evidence instead of leaving the component row
...
## Acceptance Criteria
- [x] `web/src/components/cc-data-table.tsx` no longer appears as the first
      Dashboard overview `missing_test_link` row after refresh.
- [x] `CcDataTable` and the dashboard-rendered `filterBar` helper are linked
      to durable browser proof evidence.
...
## Result Report
- Task summary: closed the `cc-data-table.tsx` Dashboard overview proof-link
  gap by reusing the existing `LUC-998` browser proof instead of creating
  another UI harness.
- Files changed:
  - `docs/architecture/scanner-overrides.json`
  - `.codex/tasks/luc-1078-dashboard-overview-cc-data-table-proof-link.md`
...
- How tested:
  ...
  - external architecture-awareness refresh generated
    `2026-07-14T09:37:53.270Z` with `2966` entities / `7331` relations /
    `16499` files and materialized the exact proof relations
  - app-completion refresh dropped `missingTestLink` from `1130` to `1127`,
    cleared the routed `cc-data-table.tsx` row, and advanced to the next
    dashboard shared-component gap
  - Project Truth advanced its first routed gap from
    `Dashboard overview: cc-data-table.tsx has app-completion risk missing_test_link.`
    to `Dashboard overview: cc-field.tsx has app-completion risk missing_test_link.`
  - `npm run architecture:status` returned `GREEN`
- What is incomplete:
  - broader Dashboard overview shared-component proof gaps remain for
    `cc-field.tsx`, `cc-notice.tsx`, `cc-resource-selector.tsx`,
    `cc-route-loading.tsx`, `cc-text-input.tsx`, and `AssetsOverview`
- Next steps:
  - if this lane continues, reuse the same `LUC-998` proof surface for
    `cc-field.tsx` as the next routed Dashboard overview shared-component
    `missing_test_link` repair
- Decisions made:
  - treated the existing `LUC-998` focused browser proof as the canonical
    evidence source for the shared dashboard/public-home managed-table family
