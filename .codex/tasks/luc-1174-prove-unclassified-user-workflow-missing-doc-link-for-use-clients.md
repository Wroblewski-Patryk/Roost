...
## Header
- ID: LUC-1174
- Title: Unclassified user workflow `USE /clients` doc-link closure
- Task Type: research
- Current Stage: verification
- Status: DONE
- Owner: Product Docs
- Depends on: [LUC-1169](/LUC/issues/LUC-1169)
- Priority: P1
- Coverage Ledger Rows: Unclassified user workflow `src/app.ts#/clients` `missing_doc_link`
- Module Confidence Rows: `Clients documentation linkage`
- Requirement Rows: not applicable
- Quality Scenario Rows: not applicable
- Risk Rows: unclassified route documentation-link drift for protected Clients aliases
- Iteration: 1
- Operation Mode: TESTER
- Mission ID: LUC-1174-USE-CLIENTS-DOC-LINK
- Mission Status: DONE
...
- Mission objective: close the routed Project Truth `missing_doc_link` gap for
  `src/app.ts#/clients` by documenting the protected Clients endpoint family in
  an accepted API contract, linking the exact route mount to that doc, and
...
## Context
`LUC-1169` already proved the exact `src/app.ts#/clients` endpoint with
...
the accepted API contract explicit for the protected Clients aliases and link
the exact route mount to that doc.

## Goal
Document the protected `/v1/clients` and compatibility `/clients` route family
in the accepted API contract, link `src/app.ts#/clients` to that contract,
refresh generated truth, and prove the `missing_doc_link` row is cleared.

## Scope
- `docs/API.md`
- `docs/architecture/relations/documentation-links.csv`
- `.codex/tasks/luc-1174-prove-unclassified-user-workflow-missing-doc-link-for-use-clients.md`
- `.codex/tasks/luc-1174-completion-evidence.md`
...
## Implementation Plan
1. Confirm the generated first gap is the exact `src/app.ts#/clients`
   `missing_doc_link` row.
2. Refine `docs/API.md` so the Clients section explicitly covers both `/v1`
   and compatibility aliases plus workspace-scoped behavior.
3. Link `src/app.ts#/clients` to that contract in
   `docs/architecture/relations/documentation-links.csv`.
...
- Issues: Project Truth routes `src/app.ts#/clients` as the first unclassified
  `missing_doc_link`.
- Gaps: `docs/API.md` listed only compatibility `/clients` routes and the
...
- Sources scanned: `src/app.ts`, `src/modules/clients/clients.routes.ts`,
  `docs/API.md`, `docs/architecture/relations/documentation-links.csv`,
  `docs/status/app-completion-index.md`, `docs/status/project-truth-index.json`,
...
- Rows created or corrected: `docs/architecture/relations/documentation-links.csv`
  now includes the exact `src/app.ts#/clients` documentation relation.
- Assumptions recorded: `docs/API.md` is the accepted source-of-truth surface
  for protected route contracts like Clients, Agents, and Dashboard.
- Blocking unknowns: none
- Why it was safe to continue: the gap is documentation-link drift, not missing
  runtime evidence.

### 2. Select One Priority Mission Objective
- Selected task: `USE /clients` missing-doc-link closure (first gap).
...
- Logic: use the existing API contract to document the protected Clients
...
- Implementation notes: `docs/API.md` now documents the protected
  `/v1/clients` routes alongside the compatibility `/clients` aliases and their
  workspace-scoped behavior; the documentation-links registry now links the
  exact `src/app.ts#/clients` mount to that accepted API contract.
...
- Result: PASS. The first parallel refresh wave left stale `missing_doc_link`
  output because dependent generators read the pre-refresh graph; rerunning
  app-completion and Project Truth sequentially against the refreshed
  `docs/graphs/architecture-awareness.json` cleared `src/app.ts#/clients`.
  App-completion now reports `missingDocLink=0`, and Project Truth advances the
  first gap to `src/app.ts#/commercial-exceptions` `missing_test_link`.
...
- Scalability assessment: the fix follows the same narrow doc-link pattern used
  for other exact route-mount closures.
- Refinements made: scope stayed on one exact route family and avoided broader
  CRM doc cleanup.
...
- [x] An accepted source-of-truth doc explicitly describes protected
  `/v1/clients` behavior plus the compatibility `/clients` aliases.
- [x] `docs/architecture/relations/documentation-links.csv` links the exact
  `src/app.ts#/clients` symbol to that doc.
- [x] Refreshed generated truth no longer reports `src/app.ts#/clients` as
  `missing_doc_link`.
...
## Completion Evidence
- Completion artifact: `.codex/tasks/luc-1174-completion-evidence.md`
...
- Verification summary:
  ...
  - `node C:/Personal/Projekty/Aplikacje/Paperclip_Softwarehouse/scripts/build-architecture-awareness-index.mjs --project Roost --root C:/Personal/Projekty/Aplikacje/Roost` PASS (`3036` entities / `7686` relations / `16523` files)
  - `node C:/Personal/Projekty/Aplikacje/Paperclip_Softwarehouse/scripts/build-app-completion-index.mjs --project Roost --root C:/Personal/Projekty/Aplikacje/Roost` PASS (`46` items / `4` flows / `31` missing test links / `0` missing doc links / `0` implemented-needs-proof / `0` blocked)
  - `ROOST_PUBLIC_URL=https://roost.luckysparrow.ch ROOST_API_PUBLIC_URL=https://api.roost.luckysparrow.ch node C:/Personal/Projekty/Aplikacje/Paperclip_Softwarehouse/scripts/build-project-truth-indexes.mjs --project Roost --root C:/Personal/Projekty/Aplikacje/Roost --apply` PASS at `2026-07-14T23:10:57.260Z` with public probes `pass`; first routed gap advanced to `src/app.ts#/commercial-exceptions` `missing_test_link`
  - `npm run architecture:status` PASS (`GREEN`, `455/769/35`)
- Residual risk: the exact `/clients` doc gap is closed; the next routed
  unclassified verification gap is `src/app.ts#/commercial-exceptions`
