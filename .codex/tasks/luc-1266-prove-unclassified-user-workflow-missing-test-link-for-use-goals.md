...
## Header
- ID: LUC-1266
- Title: Prove unclassified user workflow missing-test-link for `USE /goals`
- Task Type: fix
...
- Iteration: 2026-07-15-LUC-1266
- Operation Mode: TESTER
- Mission ID: LUC-1266-USE-GOALS-PROOF-LINK
...
## Mission Block
- Mission objective: close the routed Project Truth `missing_test_link` gap for `src/app.ts#/goals` by linking the existing `/v1/goals` API proof to the exact route mount and refreshing generated truth artifacts.
...
- Explicit exclusions: no route logic change, no auth model change, no deploy, no production mutation, no new browser work, and no broader goals feature redesign.
...
## Context
Project Truth currently routes `src/app.ts#/goals` (`USE /goals`) as the first unclassified `missing_test_link` item. The behavior is already exercised in `src/tests/api.test.ts` through direct `/v1/goals` assertions covering foreign-workspace create denial, same-workspace create, read, update, and archive behavior. The open gap is evidence-link drift between the route mount and existing automated proof.

## Goal
Attach existing test evidence to `src/app.ts#/goals` in scanner metadata so the generated app-completion and Project Truth outputs no longer classify it as `missing_test_link`.
...
## Implementation Plan
1. Verify the dispatched route symbol and existing `/v1/goals` proof in code and generated status artifacts.
2. Link `src/app.ts#/goals` to the existing API proof through a minimal scanner override.
...
## Acceptance Criteria
- `docs/architecture/scanner-overrides.json` contains a `verified` entry for `src/app.ts#/goals` with existing API proof evidence.
- Focused local proof confirms the current `/v1/goals` assertions still pass without runtime modification.
- Generated app-completion no longer reports `api_endpoint:use-goals:da30547c55` as `missing_test_link`.
- Project Truth first gap moves away from `USE /goals`.
...
- Project Truth first gap: Unclassified user workflow `USE /goals`.
- Evidence status: `src/tests/api.test.ts` already proves cross-workspace create denial plus same-workspace create, read, update, and archive behavior for `/v1/goals`.
- Closure risk: route-mount evidence drift, not missing runtime behavior.

### 2. Select Priority Mission Objective
- Selected mission: `USE /goals` missing-test-link closure for the exact `src/app.ts#/goals` mount.
- Deferred scope: no behavioral additions and no broader goals feature work.

### 3. Execute Implementation
- Scanner override links `src/app.ts#/goals` to existing proof in `src/tests/api.test.ts`, `src/modules/goals/goals.routes.ts`, and this task packet.
...
- Scope: completed the exact `missing_test_link` proof-link closure for
  `src/app.ts#/goals` only.
- Evidence:
  - `docs/architecture/scanner-overrides.json` marks `src/app.ts#/goals`
    `verified` through `src/tests/api.test.ts`,
    `src/modules/goals/goals.routes.ts`, and this task packet.
  - Focused protected API proof PASS after local build, migrations, and seed:
    ...
    `companycore-test-postgres-luc1266` on port `58000`; container removed
    during cleanup.
  - `npm run architecture:refresh` PASS.
  - External architecture-awareness refresh generated
    `2026-07-15T18:09:04.878Z` with `3060` entities / `7912` relations /
    `16523` files and materialized the exact proof relations.
  - Sequential app-completion refresh PASS with `46` items / `4` flows / `23`
    missing test links / `2` missing doc links / `0`
    implemented-needs-proof / `0` blocked; `USE /goals` no longer appears as
    `missing_test_link` and now resolves as the same-symbol docs-owned
    `missing_doc_link`.
  - Sequential Project Truth apply generated `2026-07-15T18:09:22.034Z` with
    public probes `pass`; the first routed gap now keeps `src/app.ts#/goals`
    only as docs-owned `missing_doc_link`, and the next QA-owned routed proof
    gap advances to `src/app.ts#/health` as `missing_test_link`.
...
- Cleanup:
  - Validation-owned PostgreSQL container
    `companycore-test-postgres-luc1266` was removed after the proof run.
  - Windows excluded TCP port ranges blocked the usual `55432-55557` test
    window, so the disposable validation database was moved to `58000`.
- Source-control closure:
  - Completed in
    `.codex/tasks/luc-1269-source-control-closure-for-luc-1266-goals-proof-link-packet.md`.
- Residual risk: the route-level QA proof gap for `src/app.ts#/goals` is
  closed, but the same symbol is now docs-owned `missing_doc_link`; the next
  QA proof lane is unclassified `src/app.ts#/health`.
