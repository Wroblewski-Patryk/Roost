...
## Header
- ID: LUC-1239
- Title: Unclassified user workflow `USE /departments` proof-link closure
- Task Type: fix
...
- Iteration: 2026-07-15-LUC-1239
- Operation Mode: TESTER
- Mission ID: LUC-1239-USE-DEPARTMENTS-PROOF-LINK
...
- Mission objective: close the routed Project Truth `missing_test_link` gap for
  `src/app.ts#/departments` by linking the existing `/v1/departments` API
  proof to the exact route mount and refreshing generated truth artifacts.
...
- Explicit exclusions: no route logic change, no auth model change, no deploy,
  no production mutation, no new browser work, and no broader departments
  feature redesign.
...
## Context
Project Truth currently routes `src/app.ts#/departments` (`USE /departments`)
as the first unclassified `missing_test_link` item. The behavior is already
exercised in `src/tests/api.test.ts` through direct `GET /v1/departments`,
`POST /v1/departments`, and `PATCH /v1/departments/:id` assertions, including
default department hydration, available-view exposure, invalid linked-view
rejection, and workspace-B isolation. The open gap is evidence-link drift
between the route mount and existing automated proof.

## Goal
Attach existing test evidence to `src/app.ts#/departments` in scanner metadata
...
## Implementation Plan
1. Verify the dispatched route symbol and existing `/v1/departments` proof in
   code and generated status artifacts.
2. Link `src/app.ts#/departments` to the existing API proof through a minimal
   scanner override.
...
- `docs/architecture/scanner-overrides.json` contains a `verified` entry for
  `src/app.ts#/departments` with existing API proof evidence.
- Focused local proof confirms the current `/v1/departments` assertions still
  pass without runtime modification.
- Generated app-completion no longer reports
  `api_endpoint:use-departments:876f72fd71` as `missing_test_link`.
- Project Truth first gap moves away from `USE /departments`.
...
- Project Truth first gap: Unclassified user workflow `USE /departments`.
- Evidence status: `src/tests/api.test.ts` already proves list, create, update,
  invalid linked-view rejection, default hydration, and cross-workspace
  isolation behavior for `/v1/departments`.
- Closure risk: route-mount evidence drift, not missing runtime behavior.

### 2. Select Priority Mission Objective
- Selected mission: `USE /departments` missing-test-link closure for the exact
  `src/app.ts#/departments` mount.
- Deferred scope: no behavioral additions and no broader departments feature
  work.

### 3. Execute Implementation
- Scanner override links `src/app.ts#/departments` to existing proof in
  `src/tests/api.test.ts`, `src/modules/departments/departments.routes.ts`,
...
- Scope: completed the exact `missing_test_link` proof-link closure for
  `src/app.ts#/departments` only.
- Evidence:
  - `docs/architecture/scanner-overrides.json` marks `src/app.ts#/departments`
    `verified` through `src/tests/api.test.ts`,
    `src/modules/departments/departments.routes.ts`, and this task packet.
  - Focused protected API proof PASS after local build, migrations, and seed:
    ...
    `companycore-test-postgres-luc1239` on port `55437`; container removed
    during cleanup.
  - `npm run architecture:refresh` PASS.
  - External architecture-awareness refresh generated
    `2026-07-15T03:39:07.585Z` with `3051` entities / `7840` relations /
    `16523` files and materialized the exact proof relations.
  - Sequential app-completion refresh PASS with `46` items / `4` flows / `25`
    missing test links / `2` missing doc links / `0`
    implemented-needs-proof / `0` blocked; `USE /departments` no longer
    appears as `missing_test_link` and now resolves as the same-symbol
    docs-owned `missing_doc_link`.
  - Sequential Project Truth apply generated `2026-07-15T03:39:38.143Z` with
    ...
    `src/app.ts#/events` as `missing_test_link`.
