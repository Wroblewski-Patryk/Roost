...
- [ ] Behavior is reproducible by another agent or developer using the recorded
  validation steps.
- [ ] Repository delivery contract and requirements traceability pass
  (`npm run codex:check`).
- [ ] Architecture and execution-chain evidence relevant to the changed
  component is inspected and updated in its canonical document; a document or
  generated report alone does not prove runtime behavior.
