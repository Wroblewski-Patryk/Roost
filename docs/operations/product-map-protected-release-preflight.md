...
| PMAP-REL-G04 Config, persistence, migration, and retention | Names-only inventory for `PRODUCT_MAP_PAPERCLIP_SOURCE_URL`, `PRODUCT_MAP_PAPERCLIP_READ_KEY`, `PRODUCT_MAP_ROOST_INGEST_URL`, `PRODUCT_MAP_ROOST_INGEST_KEY`, and conditional signing binding; Paperclip/Roost/Security secret-store owners; bootstrap/new-key proof/revoke-old rotation order; exact ingest URL and allowlist ownership; migration diff; active pointer plus immutable accepted/LKG, quarantine/conflict, idempotency receipt tables; 30/90-day retention, one-year audit retention, daily bounded cleanup and alert; backup/restore of projection tables; ingress proof that rejects content encoding, caps raw body before parsing at `256 KiB`, uses one in-flight ingest per workspace with no queue, enforces six requests/minute per key/workspace with burst three, returns generic no-fact denials, and bypasses shared cache/CDN with `Cache-Control: private, no-store` plus `Vary: Authorization`; no secret values | TSA + DRE + DB/Security when applicable | contract selected in architecture; implementation migration, binding presence, ingress-control, and cleanup/restore evidence remain required |
| PMAP-REL-G05 Paperclip and publisher runtime acceptance | Zero active Paperclip runs; exact owner binding; controlled canonical restart; source route and denial matrix proving that board/session/agent/run/broad credentials are rejected before source loaders run; existing canonical supervisor service `roost-product-map-publisher` start/restart/stop and safe cancellation; five-minute schedule, single-run lock/coalescence, 3/10-second timeouts, bounded retry, replay/idempotency, pinned DNS/TLS/no-proxy checks, TCP/443 allowlist proof; supervisor health/log/topology audit; rollback package readback | TSA + DRE | contract selected in architecture; protected restart and runtime proof are not authorized/performed |
...
| PMAP-REL-G07 Post-deploy transport smoke | Public health and SHA, API metadata, authenticated Product Map browser journey, separate ingest/read authorization denials, same-semantic-snapshot fresh-observation acceptance, exact retry, same-observation different-digest conflict, content-encoding/oversize/concurrency/rate-limit generic denial checks, source-based stale/LKG/unavailable labeling, five-minute publisher delivery, responsive/accessibility checks, redacted logs, and data/migration verification | DRE + QA/Security | blocked until implementation and deployment |
...
| --- | --- | --- | --- |
| PMAP-OPS-01 Publisher runtime ownership | Use the existing canonical Paperclip runtime supervisor with one named child service `roost-product-map-publisher`; package commit, working directory, least-privilege identity, five-minute tick, single-run lock/coalescence, bounded restart, supervisor health/log sink, atomic upgrade/rollback, and graceful cancellation are fixed by the architecture contract. No second instance, listener, fallback port, or unmanaged watcher. | TSA defines the runtime boundary; DRE accepts it | selected; exact package commit and service readback are implementation evidence for G01/G03/G05/G08 |
| PMAP-OPS-02 Network and binding ownership | Use the exact approved HTTPS host/path on TCP/443 with no redirects/proxy inheritance and per-connection DNS/TLS/private-address rejection; record outbound and ingress allowlist owners; use the five names-only bindings and approved stores; bootstrap, two-key rotation, compromise disablement, and conditional signature decision are explicit. Source-route approval does not authorize Roost ingress. | TSA + Security define; DRE verifies | selected; reachability, allowlist, binding presence, and final signature verdict remain gated evidence |
| PMAP-OPS-03 Projection-state lifecycle | Use versioned durable workspace-partitioned active pointer, accepted/LKG, quarantine/conflict, and idempotency receipt records; enforce re-observation/idempotency/conflict/order/rollback rules; retain accepted/receipts 30 days, quarantine/conflicts 90 days, audit one year; run bounded daily cleanup with failure alert; back up/restore projection tables and never move the active pointer backward. | TSA + DB Engineering define; DRE verifies recovery | selected; migration, cleanup, backup/restore, and monitoring evidence remain gated |
| PMAP-OPS-04 Multi-artifact provenance | Bind one immutable packet to Paperclip source commit/route/schema, publisher package commit/digest/service definition/rollback package, Roost candidate/migrations, transport/schema versions, names-only bindings, and known-good Roost commit/image. G01/G06/G08 read back the same packet; any mismatch is NO-GO. | Engineering Delivery + TSA + DRE | selected; candidate-specific packet and readback remain gated |
