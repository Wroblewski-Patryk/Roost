...
## Graph Layer Reconciliation

Roost intentionally has two architecture graph layers with different scope.

| Layer | Source artifacts | Current scale | Use for | Not for |
| --- | --- | ---: | --- | --- |
| Project-native evidence graph | `docs/architecture/nodes/*.csv`, `docs/architecture/relations/dependencies.csv`, `docs/architecture/chains/chains.csv`, `docs/testing/test-map.csv`, `docs/status/evidence-status.csv`, generated into `docs/graphs/project-graph.json` and `docs/status/architecture-health-dashboard.md` | `454` nodes / `765` relations / `35` chains as of the 2026-06-20 status refresh | Canonical architecture gates, evidence status, chain coverage, registry drift, and release confidence | Counting every inferred function, document, or task found by the broad repository scanner |
| Paperclip architecture-awareness scanner export | `docs/graphs/architecture-awareness.json`, `docs/graphs/architecture-awareness.csv`, `docs/graphs/architecture-health.json`, `docs/status/architecture-awareness-report.md`, generated by `build-architecture-awareness-index.mjs` | `2406` entities / `5036` relations as of the 2026-06-20T20:13:23.962Z scanner refresh | Broad inferred inventory, task-link backfill, owner attribution, missing inferred test/doc signals, and scanner hygiene | Replacing the project-native evidence graph or failing the architecture gate solely because its counts are larger |

The count difference between these layers is expected scanner-scope layering,
not drift by itself. Treat it as drift only when a concrete entity required by
one layer is missing or contradictory in the other layer. The project-native
graph remains the gate source for `npm run architecture:status`; the scanner
export remains an auxiliary awareness and task-linking source.

Canonical interpretation as of [LUC-5302](/LUC/issues/LUC-5302): a GREEN
`npm run architecture:status` result with evidence queue `0`, chain worklist
`0`, and all gates passing is compatible with the awareness scanner reporting
`implementation_without_tests=1162` / actionable `1153`. That scanner signal
is acceptable confidence debt, not a docs-model mismatch and not a release or
QA planning blocker by itself, while task links, owner attribution, docs links,
verified-proof links, and disconnected-entity checks remain clean. PM and QA
agents should turn the signal into named proof-ladder selections from module
risk, then create a repair task only when a focused proof finds a real defect
or when a required canonical evidence row/chain is missing from the
project-native graph.

Known-state refresh policy as of [LUC-5247](/LUC/issues/LUC-5247): use
`--status-only` as the heartbeat preflight, then run a full scanner refresh
only when needed with `--max-elapsed-ms 180000 --progress-every 5000` for the
current Roost workspace size. If the full refresh exceeds that budget, treat
the existing scanner exports as stale-but-preserved, record the failed phase,
and open a focused scanner optimization or cache lane instead of running an
unbounded heartbeat command.
...
- graph: `467` nodes, `775` relations, `36` chains
- evidence queue: `0`
- chain hardening worklist: `0`
- chain coverage gate: `35/35` features (`100%`)
