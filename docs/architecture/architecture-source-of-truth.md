...
- the unchanged projection `schemaVersion`;
...
- `packetDigest` as SHA-256 over canonical packet bytes;
...
- A different digest for the same
  `(companyId, schemaVersion, sourceSnapshotId)` is a conflict. The new packet
  is quarantined, the previous active packet remains the last-known-good view,
  and the UI shows conflict/stale state. No newest-wins rule is allowed.
