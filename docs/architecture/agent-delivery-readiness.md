...
Aktualizacja v34: [kanoniczne źródła bootstrap](worker-bootstrap-authority-source-v1.md)
— **DONE dla odczytu i testów odmów: 13/13 nowych wyników, 99/99 łącznie**.
Odczyt istniejących rekordów działa w transakcji rejestru; nie tworzy drugiego
źródła prawdy. Dziewięć brakujących faktów strukturalnych utrzymuje autoryzację
bootstrap jako BLOCKED. Bez wymyślania epok, zmian bazy, sieci i aktywacji;
wszystkie flagi false. Jeden następny proponowany atom: źródłowy kontrakt cyklu
życia hosta/instalacji i wspólnej blokady zapisu w istniejących encjach, bez
inicjalizacji brakujących faktów. Wcześniejsze dowody i propozycje są historyczne.
